...
# 三点五、添加系统规则中的包名到 target.txt
SYS_RULE_FILE="$MODDIR/系统添加规则.txt"
if [ -f "$SYS_RULE_FILE" ]; then
  ui_print "📎 正在读取系统添加规则.txt..."

  while IFS= read -r pkg || [ -n "$pkg" ]; do
    [ -z "$pkg" ] && continue  # 跳过空行
    # 检查是否已存在（原样、加!、加? 都不重复添加）
    grep -q -x "$pkg" "$TARGET_FILE" && continue
    grep -q -x "$pkg!" "$TARGET_FILE" && continue
    grep -q -x "$pkg?" "$TARGET_FILE" && continue

    case "$CURRENT_MODE" in
      0) echo "$pkg" >> "$TARGET_FILE" ;;
      1) echo "$pkg!" >> "$TARGET_FILE" ;;
      2) echo "$pkg?" >> "$TARGET_FILE" ;;
    esac
  done < "$SYS_RULE_FILE"

  ui_print "✅ 已按模式 $CURRENT_MODE 自动添加系统规则至 target.txt"
else
  ui_print "ℹ️ 未找到系统添加规则.txt，跳过自动导入"
fi
# 四、显示前5个加载的包名
