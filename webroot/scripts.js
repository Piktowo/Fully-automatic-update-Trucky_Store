...
runNowBtn.addEventListener("click", async () => {
    ...
    // [1] 检查两个文件并找出交集
        try {
            const [whitelistRaw, targetRaw] = await Promise.all([
                exec("cat /data/adb/modules/Tricky_Store-xiaoyi/conf/白名单.txt"),
                exec("cat /data/adb/tricky_store/target.txt")
            ]);

            const whitelist = whitelistRaw.trim().split("\n").map(x => x.trim()).filter(Boolean);
            const targetList = targetRaw.trim().split("\n")
                .map(x => x.trim().replace(/[!?]$/, "")) // 去除末尾的!或?
                .filter(Boolean);

            const commonPackages = whitelist.filter(pkg => targetList.includes(pkg));

            if (commonPackages.length > 0) {
                appendToOutput(`[${new Date().toLocaleTimeString()}] `);
                appendToOutput("已删除包名：");
                for (const pkg of commonPackages) {
                    appendToOutput(pkg);
                }
                appendToOutput("因为以上包名存在于白名单中");
                appendToOutput("");
                appendToOutput("白名单路径:/data/adb/modules/Tricky_Store-xiaoyi/conf/白名单.txt");
                appendToOutput("");
            } else {
                appendToOutput(`[${new Date().toLocaleTimeString()}] `);
                appendToOutput("已按照白名单排除包名");
                appendToOutput("");
            }

        } catch (err) {
            appendToOutput(`[${new Date().toLocaleTimeString()}] `);
            appendToOutput("[!] 获取包名列表失败，仍将尝试执行更新");
            appendToOutput("");
            console.error("比较包名时出错：", err);
        }
